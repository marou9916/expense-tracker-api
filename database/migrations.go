...
func applyMigrations(db *sql.DB) {
	...
	ALTER TABLE user ADD COLUMN id_user_uuid UUID DEFAULT uuid_generate_v4();

	ALTER TABLE expense ADD COLUMN id_user_uuid UUID NOT NULL;

	ALTER TABLE expense ADD CONSTRAINT fk_id_user_uuid FOREIGN KEY (id_user_uuid) REFERENCES user(id_user_uuid);

	ALTER TABLE user DROP COLUMN id_user;

	ALTER TABLE user ADD CONSTRAINT pk_id_user_uuid PRIMARY KEY(id_user_uuid);
